...
def get_new_live_user():
    ...
    for item in data.get("data", []):
        ...
        return username  # take first live result
...
class LiveTracker:
    ...
    def register_events(self):
        ...
        async def on_gift(event):
            ...
            diamonds = getattr(event.gift, "diamond_count", 0) or 0
    ...
    async def handle_live_end(self):
        ...
        await start_new_tracker()

    async def start(self):

        while True:
            try:
                print(f"🔄 Connecting to {self.username}")
...
async def start_new_tracker():
    ...
    new_user = get_new_live_user()

    if not new_user:
        print("⚠️ No live users found. Retrying in 60s...")
        await asyncio.sleep(60)
        await start_new_tracker()
        return

    print(f"🎯 Switching to new live user: {new_user}")

    current_tracker = LiveTracker(new_user)
    asyncio.create_task(current_tracker.start())
