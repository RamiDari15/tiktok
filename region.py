...
COUNTRY_CODES = {
    ...
    "LT": "Lithuania", "LU": "Uinted Kingdom", "MO": "Macao", "MG": "Madagascar", "MW": "Malawi", "MY": "Malaysia",
...
def extract_user_data(html):
    ...
    region = user_info.get("region") or user_info.get("location") or "N/A"
...
def fetch_with_retry(url, base_headers, max_retries=3, use_cookie=False):
    for attempt in range(max_retries):
        try:
            ...
            response = requests.get(url, headers=current_headers, impersonate="chrome110", timeout=15)
...
def process_csv(input_file, output_file):
    ...
    with open(input_file, mode='r', encoding='utf-8') as infile:
        ...
        fieldnames = reader.fieldnames + ['detected_region', 'region_name']
        
        with open(output_file, mode='w', encoding='utf-8', newline='') as outfile:
            writer = csv.DictWriter(outfile, fieldnames=fieldnames)
            writer.writeheader()
            
            print(f"[*] Starting scan. Filtering for: {allowed_regions}\n")
            
            for row in reader:
                # Expecting a column named 'username' in your CSV
                raw_user = row.get('username', '')
                username = extract_username_from_input(raw_user)
                
                if not username:
                    continue
                
                print(f"[*] Checking @{username}...", end="\r")
                
                try:
                    data = fetch_tiktok_profile(username)
                    if data:
                        region_code = data.get('region', '').upper()
                        if region_code in allowed_regions:
                            row['detected_region'] = region_code
                            row['region_name'] = COUNTRY_CODES.get(region_code, "Unknown")
                            writer.writerow(row)
                            print(f"[+] MATCH: @{username} is in {region_code}      ")
                        else:
                            print(f"[-] SKIP: @{username} is in {region_code}       ")
                    else:
                        print(f"[!] FAILED: Could not access @{username}      ")
...
if __name__ == "__main__":
    print("="*40)
    print("  TIKTOK REGION FILTER (USA & CANADA)")
    print("="*40)
    
    # Configuration
    INPUT_FILE = "live_usernames.csv"       # Name of your source file
    OUTPUT_FILE = "filtered.csv"   # Name of the results file
